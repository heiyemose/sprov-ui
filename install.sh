...
install_sprov-ui() {
    ...
    wget -N --no-check-certificate -O /usr/local/sprov-ui/sprov-ui.jar https://github.com/heiyemose/sprov-ui/blob/master/sprov-ui.jar
    if [[ $? -ne 0 ]]; then
    ...
    echo -e "wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/heiyemose/sprov-ui/blob/master/sprov-ui.sh && chmod +x /usr/bin/sprov-ui"
    echo -e ""
    echo -e "若未安装 bbr 等加速工具，推荐使用以下命令一键安装 bbr："
    echo -e "wget --no-check-certificate https://github.com/heiyemose/sprov-ui/blob/master/bbr.sh && bash bbr.sh"
    echo -e ""
}
